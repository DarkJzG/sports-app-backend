# flask_api/controlador/control_pantaloneta_ia_v1.py
...
def calcular_costo_produccion_pantaloneta(atributos: dict) -> dict:
    """Calcula el costo de producción de una pantaloneta"""
    # Costo base según la tela
    if atributos.get("tela") == "Poliéster":
        costo_material = 2.5
    elif atributos.get("tela") == "Microfibra":
        costo_material = 3.0
    elif atributos.get("tela") == "Mesh":
        costo_material = 2.8
    else:
        costo_material = 2.5
    
    # Ajuste por largo
    if atributos.get("largo") == "largo":
        costo_material += 0.5  # Más tela
    ...
    if camino == "paneles":
        costo_diseno = 1.2  # Costura de paneles
    elif camino == "sublimacion":
        area = atributos.get("areaDisenoIA", "completo")
        if area == "completo":
            costo_diseno = 2.5  # Sublimación completa
        else:
            costo_diseno = 1.8  # Sublimación parcial
    else:
        costo_diseno = 0.8  # Diseño simple
...
def generar_pantaloneta_v1(categoria_id, atributos_es, user_id):
    """
    Genera la imagen IA de una pantaloneta y guarda la prenda en la base de datos.
    """
    STABLE_URL = current_app.config.get("STABLE_URL", "http://127.0.0.1:7860")
    
    print("\n==============================")
    print("🩳 INICIO generar_pantaloneta_v1")
    print("==============================")
    ...
    print("\n🧩 Entrando a build_prompt_pantaloneta_v1 con:", atributos_en)
    prompt_en = build_prompt_pantaloneta_v1(atributos_en)
    print("🟣 Prompt generado:\n", prompt_en, "\n")
    
    descripcion_es = descripcion_pantaloneta_es_v1(atributos_es)
    ...
    payload = {
        ...
        "width": 512,
        "height": 512,  # Formato cuadrado para pantalonetas
        "sampler_name": "DPM++ 2M",
        "steps": 30,
        "cfg_scale": 7,
        "seed": -1,
    ...
    cloud = cloudinary.uploader.upload(image_bytes, folder="Pantaloneta_V1")
    image_url = cloud.get("secure_url")
    print("✅ Imagen subida:", image_url)
    
    # 6️⃣ Calcular costo
    costo = calcular_costo_produccion_pantaloneta(atributos_es)
    ...
    print("\n✅ Pantaloneta guardada exitosamente")
    print("==============================\n")
